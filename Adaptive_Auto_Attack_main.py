...
def margin_loss(logits, y, i=0, MT=True):
    ...
    if MT:
        ...
        LTA_s = (
            torch.tensor([LTA_k[1][s][i % TA.shape[-1]] for s in range(bs)])
            .view(-1, 1)
            .to("cuda")
        )
...
def AAA_white_box(
    ...
    randVector_ = (
        torch.FloatTensor(X_adv.shape[0], num_class).uniform_(-1.0, 1.0).to(device)
    )
    if bias_ODI and out_re > 1:
        if BIAS_ATK:
            randVector_[np.arange(X_adv.shape[0]), y] = (
                num_class / 10
            ) * random.uniform(-0.1, 0.5)
            no_r = -2 - No_class % threshold_class
            randVector_[np.arange(X_adv.shape[0]), sorted_logits[:, no_r]] = (
                num_class / 10
            ) * 0.8
        else:
    ...
    for i in range(ADI_step_num + step_num):
        ...
        with torch.enable_grad():
            if i < ADI_step_num:
                ...
                if MT:
                    loss = margin_loss(
                        model(X_adv_f), y[atk_filed_index], No_epoch, MT=MT
                    ).sum()
                else:
        ...
        if i < ADI_step_num:
            ...
            step_size = (
                min_ssize
                + (max_ssize - min_ssize)
                * (1 + math.cos(math.pi * s / (step_num + 1)))
                / 2
            )
            if Lnorm == "Linf":
                if X_adv_f.grad is None:
        ...
        if Lnorm == "Linf":
            ...
            X_adv_f = torch.clamp(
                X_f
                + normalize_x(X_adv_f - X_f)
                * torch.min(
                    epsilon * torch.ones_like(X_f).detach(), lp_norm(X_adv_f - X_f)
                ),
                0.0,
                1.0,
            )
        ...
        each_max_loss[np_atk_filed_index] = np.where(
            each_max_loss[np_atk_filed_index] > output_loss.detach().cpu().numpy(),
            each_max_loss[np_atk_filed_index],
            output_loss.detach().cpu().numpy(),
        )
    ...
    return (
        ...
        odi_atk_suc_num,
        each_back_num,
        total_odi_distribution,
        tfn,
    )


def Adaptive_Auto_white_box_attack(
    model,
    device,
    eps,
    is_random,
    ...
    init_atk = pd.DataFrame(
        {
            "img_path": all_atk_imgs,
            "label": all_atk_labs,
            "not_suc": not_suc_index,
            "need_atk": not_suc_index,
        }
    )
    data_loader = mytest_loader(
        batch_size=batch_size,
        dataframe=init_atk,
        datasets=data_set,
    ...
    for i, test_data in enumerate(data_loader):
        ...
        if Lnorm == "Linf":
            random_noise = eps * torch.FloatTensor(*X.shape).uniform_(
                -eps, eps
            ).sign().to(device)
        ...
        sorted_logits_index[bstart:bend] = np.argpartition(
            out_adv.detach().cpu().numpy(),
            np.argmin(out_adv.detach().cpu().numpy(), axis=-1),
            axis=-1,
        )[:, -logits_num:]
    ...
    for out_re in trange(out_restart_num):
        ...
        if out_re == 2:
            ...
            print(
                f"{model_name} ##################### wrong_dire:{wrong_logits_direct:0.4} "
        ...
        for r in range(restart_num):
            ...
            need_atk_num_place1 = (len(not_suc_index) - now_not_suc_num) + min(
                max(100, int(now_not_suc_num * alpha)), now_not_suc_num
            )
            ...
            data_loader = mytest_loader(
                batch_size=batch_size,
                dataframe=fast_init_atk,
                datasets=data_set,
                model_name=model_name,
            )
            if (out_re < 10 and BIAS_ATK == False) or (
                out_re < 11 and BIAS_ATK == True
            ):
                final_odi_atk = False
            else:
                final_odi_atk = True
            ...
            iter_num = int(
                ...
                / (restart_num * (out_restart_num - out_re))
                * now_need_atk_num
            )
            ...
            for i, test_data in enumerate(data_loader):
                ...
                for w in range(warm):
                    ...
                    (
                        atk_filed_num_cold,
                        atk_filed_index_cold,
                        X_pgd,
                        each_max_loss,
                        odi_atk_suc_num,
                        backward_num,
                        each_odi_distribution,
                        tfn,
                    ) = AAA_white_box(
                        model,
                        X,
                        ...
                        No_class=No_class,
                        w_logit_dir=wrong_logits_direct,
                        c_logit_dir=correct_logits_direct,
                        data_set=data_set,
                        BIAS_ATK=BIAS_ATK,
                        final_bias_ODI=final_odi_atk,
                        No_epoch=No_epoch,
                        num_class=num_class,
                        MT=MT,
                        Lnorm=Lnorm,
                    ...
                    now_max_loss[bstart:bend] = np.where(
                        now_max_loss[bstart:bend] > each_max_loss,
                        now_max_loss[bstart:bend],
                        each_max_loss,
                    )
            ...
            print(
                f"No.{len(acc_curve)} clean_acc: {clean_acc:0.4} robust acc: {not_suc_index.sum() / len(all_atk_labs):0.4} used_bp_num: {used_backward_num} "
...
def main(
    flag, model_name, ep=8.0 / 255, random=True, batch_size=128, average_number=100
):
    ...
    if model_name == "MART":
        ...
        model.load_state_dict(
            torch.load("model_weights/MART/MART_UWRN.pt")["state_dict"]
        )
    ...
    if model_name == "awp_28_10":
        ...
        ckpt = filter_state_dict(
            torch.load("model_weights/AWP/RST-AWP_cifar10_linf_wrn28-10.pt")
        )
    ...
    if model_name == "awp_34_10":
        ...
        ckpt = filter_state_dict(
            torch.load("model_weights/AWP/TRADES-AWP_cifar10_linf_wrn34-10.pt")
        )
        model.load_state_dict(ckpt)
    ...
    if model_name == "fbtf":
        ...
        model.load_state_dict(
            torch.load("model_weights/FBTF/Wong2020Fast.pt"), strict=False
        )
        model = nn.Sequential(
            Normalize([0.4914, 0.4822, 0.4465], [0.2471, 0.2435, 0.2616]), model
        )
    ...
    if model_name == "geometry":
        ...
        model.load_state_dict(
            torch.load("model_weights/GEOMETRY/Zhang2020Geometry.pt")["state_dict"]
        )
    ...
    if model_name == "hydra":
        ...
        model.load_state_dict(
            torch.load(
                "model_weights/HYDRA/model_best_dense.pth.tar", map_location="cuda"
            )["state_dict"]
        )
    ...
    if model_name == "hyer_embe":
        ...
        model = torch.nn.DataParallel(model)
        model.load_state_dict(
            torch.load("model_weights/HYPER_EMBE/model-wideres-pgdHE-wide20.pt")[
                "state_dict"
            ]
        )
    ...
    if model_name == "level_sets":
        ...
        model = ResNet18().to("cuda")
        ckpt = level_sets_filter_state_dict(
            torch.load("model_weights/LEVEL_SETS/200.pth")
        )
        model.load_state_dict(ckpt)
    ...
    if model_name == "overfit":
        ...
        model = torch.nn.DataParallel(model)
        model.load_state_dict(
            torch.load("model_weights/OVERFIT/cifar10_wide20_linf_eps8.pth")
        )
        model = nn.Sequential(
            Normalize([0.4914, 0.4822, 0.4465], [0.2471, 0.2435, 0.2616]), model
        )
    ...
    if model_name == "pre_train":
        ...
        model.load_state_dict(
            torch.load("model_weights/PRE_TRAIN/cifar10wrn_baseline_epoch_4.pt")
        )
        model = nn.Sequential(Normalize([0.5, 0.5, 0.5], [0.50, 0.50, 0.50]), model)
    ...
    if model_name == "proxy_dist":
        ...
        model = torch.nn.DataParallel(model)
        model.load_state_dict(
            torch.load("model_weights/PROXY_DIST/Sehwag2021Proxy.pt")["state_dict"]
        )
    ...
    if model_name == "rst":
        ...
        model = nn.DataParallel(model)
        model.load_state_dict(
            torch.load("model_weights/RST/rst_adv.pt.ckpt")["state_dict"]
        )
    ...
    if model_name == "self_adaptive":
        ...
        model = nn.DataParallel(model)
        model.load_state_dict(
            torch.load("model_weights/self-adaptive/model-wideres-epoch78.pth")
        )
    if model_name == "sensible":
        ...
        model = torch.nn.DataParallel(model)
        model.load_state_dict(
            torch.load("model_weights/SENSIBLE/SENSE_checkpoint300.dict")["state_dict"]
        )
    ...
    if model_name == "understanding_fast":
        ...
        model.load_state_dict(
            torch.load(
                "model_weights/Understanding_FAST/Andriushchenko2020Understanding.pt"
            )["last"]
        )
    ...
    if model_name == "yopo":
        ...
        model.load_state_dict(
            torch.load("model_weights/YOPO/Zhang2019You.pt", map_location="cuda")
        )
    ...
    if model_name == "ULAT_28_10_with":
        ...
        model = model_ctor(
            ...
            depth=28,
            width=10,
            activation_fn=widresnet.Swish,
            mean=widresnet.CIFAR10_MEAN,
            std=widresnet.CIFAR10_STD,
        )
        model.load_state_dict(
            torch.load("model_weights/FIX_DATA/cifar10_linf_wrn28-10_with.pt")
        )
    ...
    if model_name == "ULAT_70_16_extra":
        ...
        model.load_state_dict(
            torch.load("model_weights/FIX_DATA/Gowal2020Uncovering_70_16_extra.pt")
        )
        batch_size = 32
    ...
    if model_name == "ULAT_34_20":
        ...
        model.load_state_dict(
            torch.load("model_weights/FIX_DATA/cifar10_linf_wrn34-20_without.pt")
        )
        batch_size = 64
    ...
    if model_name == "ULAT_70_16":
        ...
        model.load_state_dict(
            torch.load("model_weights/FIX_DATA/cifar10_linf_wrn70-16_without.pt")
        )
        batch_size = 32
    ...
    if model_name == "fix_data_28_10_with":
        ...
        model.load_state_dict(
            torch.load(
                "model_weights/FIX_DATA_V2/cifar10_linf_wrn28-10_cutmix_ddpm_v2.pt"
            )
        )
    ...
    if model_name == "fix_data_70_16_extra":
        ...
        model.load_state_dict(
            torch.load(
                "model_weights/FIX_DATA_V2/cifar10_linf_wrn70-16_cutmix_ddpm_v2.pt"
            )
        )
        batch_size = 32
    ...
    if model_name == "LBGAT_34_10":
        ...
        model = WideResNet().to(device)
        model.load_state_dict(
            torch.load("model_weights/LBGAT/cifar10_lbgat0_wideresnet34-10.pt")
        )
    ...
    if model_name == "LBGAT_34_20":
        ...
        model.load_state_dict(
            torch.load("model_weights/LBGAT/cifar10_lbgat0_wideresnet34-20.pt")
        )
    ...
    if model_name == "FAT":
        ...
        model.load_state_dict(
            torch.load(
                "model_weights/FAT/fat_for_trades_wrn34-10_eps0.062_beta6.0.pth.tar"
            )["state_dict"]
        )
    ...
    if model_name == "TRPF":
        ...
        model = torch.nn.Sequential(Normalize_layer(mean, std), model)
        # model = torch.nn.DataParallel(model)
        ckpt = torch.load(
            "model_weights/TRPF/resnet18_cifar10_dens0.05_magnitude_epoch200_testAcc_87.31999969482422.pt"
        )
        model.load_state_dict(ckpt)
    ...
    if model_name == "RLPE_28_10":
        ...
        model = nn.DataParallel(model)
        model.load_state_dict(
            torch.load("model_weights/RLPE/RST_0.1485.pt")["state_dict"]
        )
    ...
    if model_name == "RLPE_34_15":
        ...
        model = nn.DataParallel(model)
        model.load_state_dict(
            torch.load("model_weights/RLPE/Wide-RST_0.1485.pt")["state_dict"]
        )
    ...
    if model_name == "ULAT_70_16_with_100":
        ...
        model = model_ctor(
            ...
            width=16,
            activation_fn=widresnet.Swish,
            mean=widresnet.CIFAR100_MEAN,
            std=widresnet.CIFAR100_STD,
        )
        model.load_state_dict(
            torch.load("model_weights/FIX_DATA/cifar100_linf_wrn70-16_with.pt")
        )
        batch_size = 32
    ...
    if model_name == "ULAT_70_16_100":
        ...
        model.load_state_dict(
            torch.load("model_weights/FIX_DATA/cifar100_linf_wrn70-16_without.pt")
        )
        batch_size = 32
    ...
    if model_name == "fix_data_28_10_with_100":
        ...
        model.load_state_dict(
            torch.load(
                "model_weights/FIX_DATA_V2/cifar100_linf_wrn28-10_cutmix_ddpm.pt"
            )
        )
    ...
    if model_name == "fix_data_70_16_extra_100":
        ...
        model.load_state_dict(
            torch.load(
                "model_weights/FIX_DATA_V2/cifar100_linf_wrn70-16_cutmix_ddpm.pt"
            )
        )
        batch_size = 32
    ...
    if model_name == "LBGAT_34_10_100":
        ...
        model.load_state_dict(
            torch.load("model_weights/LBGAT/cifar100_lbgat6_wideresnet34-10.pt")
        )
    ...
    if model_name == "LBGAT_34_20_100":
        ...
        model = torch.nn.DataParallel(model)
        model.load_state_dict(
            torch.load("model_weights/LBGAT/cifar100_lbgat6_wideresnet34-20.pt")
        )
    ...
    if model_name == "awp_34_10_100":
        ...
        ckpt = filter_state_dict(
            torch.load("model_weights/AWP/AT-AWP_cifar100_linf_wrn34-10.pth")
        )
        model.load_state_dict(ckpt)
        model = nn.Sequential(
            Normalize(
    ...
    if model_name == "pre_train_28_10_100":
        ...
        model.load_state_dict(
            torch.load("model_weights/PRE_TRAIN/cifar100wrn_baseline_epoch_4.pt")
        )
        model = nn.Sequential(Normalize([0.5, 0.5, 0.5], [0.50, 0.50, 0.50]), model)
    ...
    if model_name == "overfit_100":
        ...
        # model = torch.nn.DataParallel(model)
        model.load_state_dict(
            torch.load("model_weights/OVERFIT/cifar100_linf_eps8.pth")
        )
    ...
    if model_name == "proxy_dist_L2":
        ...
        model = torch.nn.DataParallel(model)
        model.load_state_dict(
            torch.load("model_weights/PROXY_DIST/Sehwag2021ProxyL2.pt")["state_dict"]
        )
        Lnorm = "L2"
        batch_size = 128
        ep = 0.5
    ...
    if model_name == "overfit_R18_L2":
        ...
        model = torch.nn.DataParallel(model)
        model.load_state_dict(
            torch.load("model_weights/OVERFIT/Rice2020OverfittingL2.pt")
        )
        model = nn.Sequential(
            Normalize([0.4914, 0.4822, 0.4465], [0.2471, 0.2435, 0.2616]), model
        )
    ...
    if model_name == "fix_data_28_10_L2":
        ...
        model.load_state_dict(
            torch.load(
                "model_weights/FIX_DATA_V2/cifar10_l2_wrn28-10_cutmix_ddpm_v2.pt"
            )
        )
        Lnorm = "L2"
        batch_size = 128
    ...
    if model_name == "TRADES_mnist":
        ...
        model = SmallCNN()
        model.load_state_dict(
            torch.load("model_weights/TRADES/model_mnist_smallcnn.pt")
        )
        ep = 0.3
        batch_size = 512
    ...
    if model_name == "ULAT_mnist":
        ...
        model.load_state_dict(
            torch.load("model_weights/FIX_DATA/mnist_linf_wrn28-10_without.pt")
        )
        batch_size = 128
        ep = 0.3
